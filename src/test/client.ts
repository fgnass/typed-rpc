...
import tap from "tap";
import { rpcClient, RpcError, websocketTransport } from "../client.ts";
import type { Service } from "./service.ts";
import type { ComplexService } from "./complexService.ts";
...
tap.setTimeout(200000);

tap.test("should use websocket", async (t) => {
  type RpcClient = ReturnType<typeof rpcClient<Service>>;
  const client = await new Promise<RpcClient>((resolve) => {
    const client = rpcClient<Service>({
      url: 'n/a',
